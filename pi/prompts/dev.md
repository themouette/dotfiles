...
Use the **scout** subagent to scan the directory containing the plan file, and its parent directories up to 2 levels up. Collect:
...
2. Ask the user: **"Do you want me to call the delegate agent to update the plan based on these findings?"**

   - **If yes**: Use the **delegate** subagent to produce an updated version of the plan at `$ARGUMENTS`. Provide it with:
     ...
     After the delegate subagent responds, show the user what changed and write the updated plan to disk.
...
4. **If the plan was updated** (user chose yes above), check whether sibling plans need to be updated as a result. Spawn one **delegate** subagent per sibling plan **in parallel** (do not wait for one to finish before starting the next). Each subagent receives:
   ...
   If any sibling plans need updating, ask the user: **"Do you want me to update the affected sibling plans now?"** If yes, apply updates one plan at a time (using the **delegate** subagent for each), following the same confirmation pattern as Step 4.
...
Use the **worker** subagent to implement the plan at `$ARGUMENTS`. In the prompt, include:
